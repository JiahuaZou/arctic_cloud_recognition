...
nrow(val_set1)
nrow(test_set1)
hist(rbind(train_val_set1, test_set1)$blocks)
```
...
nrow(test_set2)
nrow(total_data2)
hist(rbind(train_val_set2, test_set2)$blocks)
```
...
CVgeneric <- function(classifier, trainFeature, trainLabel, K = 5, lossfn){
  ...
  if(!(str_to_lower(classifier) %in% c("qda", "logistic", "random forest", "knn"))){
    ...
    for(i in 1:K){
      model <- qda(trainLabel[-folds[[i]]] ~ NDAI + CORR + AN, data = trainFeature[-folds[[i]], ])
...
CVgeneric("qda", train_set1[,c(4,6,11)], train_set1$label, lossfn = loss)
CVgeneric("KNN", train_set1[,c(4,6,11)], train_set1$label, lossfn = loss)
...
# Logistic
err_logis1 <- CVgeneric("logistic", train_set1[,c(4,6,11)], train_set1$label, lossfn = loss)
err_logis1

# QDA
err_qda1 <- CVgeneric("qda", train_set1[,c(4,6,11)], train_set1$label, lossfn = loss)
err_qda1

# KNN
err_knn1 <- CVgeneric("KNN", train_set1[,c(4,6,11)], train_set1$label, lossfn = loss)
err_knn1
...
ggplot(data = train1) +
  geom_point(aes(x = NDAI, y = AN, shape = factor(label), color = factor(label))) + 
  scale_color_manual(values=c("#66c2a5", "#fc8d62"))
ggplot(data = train1) +
  geom_point(aes(x = CORR, y = AN, shape = factor(label), color = factor(label))) + 
  scale_color_manual(values=c("#66c2a5", "#fc8d62"))
